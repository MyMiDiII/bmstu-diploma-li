...
*/
typedef struct mytable_vtab mytable_vtab;
struct mytable_vtab {
...
*/
typedef struct mytable_cursor mytable_cursor;
struct mytable_cursor {
...
static int mytableConnect(
  ...
  char **pzErr
    ...
    puts("CONNECT");
    return mytableCreate(db, pAux, argc, argv, ppVtab, pzErr);
}
...
static int mytableDisconnect(sqlite3_vtab *pVtab){
    puts("DISCONNECT");
  mytable_vtab *p = (mytable_vtab*)pVtab;
  sqlite3_free(p);
  return SQLITE_OK;
}
...
*/
static int mytableOpen(sqlite3_vtab *p, sqlite3_vtab_cursor **ppCursor){
    puts("OPEN");
  mytable_cursor *pCur;
...
*/
static int mytableClose(sqlite3_vtab_cursor *cur){
    puts("CLOSE");
  mytable_cursor *pCur = (mytable_cursor*)cur;
  sqlite3_free(pCur);
  return SQLITE_OK;
}
...
*/
static int mytableNext(sqlite3_vtab_cursor *cur){
    puts("NEXT");
  mytable_cursor *pCur = (mytable_cursor*)cur;
  pCur->iRowid++;
  return SQLITE_OK;
...
*/
static int mytableRowid(sqlite3_vtab_cursor *cur, sqlite_int64 *pRowid){
    puts("ROWID");
  mytable_cursor *pCur = (mytable_cursor*)cur;
  *pRowid = pCur->iRowid;
  return SQLITE_OK;
...
** row of output.
*/
static int mytableEof(sqlite3_vtab_cursor *cur){
    puts("EOF");
  mytable_cursor *pCur = (mytable_cursor*)cur;
...
** templatevtabEof().
*/
static int mytableFilter(
  sqlite3_vtab_cursor *pVtabCursor, 
  int idxNum, const char *idxStr,
  int argc, sqlite3_value **argv
){
    puts("FILTER");
  mytable_cursor *pCur = (mytable_cursor *)pVtabCursor;
  pCur->iRowid = 1;
  return SQLITE_OK;
}
...
** virtual table.
*/
static sqlite3_module mytableModule = {
  /* iVersion    */ 0,
  /* xCreate     */ mytableCreate,
  /* xConnect    */ mytableConnect,
  /* xBestIndex  */ mytableBestIndex,
  /* xDisconnect */ mytableDisconnect,
  /* xDestroy    */ mytableDisconnect,
  /* xOpen       */ mytableOpen,
  /* xClose      */ mytableClose,
  /* xFilter     */ mytableFilter,
  /* xNext       */ mytableNext,
  /* xEof        */ mytableEof,
  /* xColumn     */ mytableColumn,
  /* xRowid      */ mytableRowid,
  /* xUpdate     */ 0,
  /* xBegin      */ 0,
  /* xSync       */ 0,
...
int sqlite3_mytable_init(
  ...
  rc = sqlite3_create_module(db, "mytable", &mytableModule, 0);
  printf("rc = %d\n", rc);
  return rc;
}
